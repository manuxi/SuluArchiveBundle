...
namespace Manuxi\SuluArchiveBundle\Common;
...
class DoctrineListRepresentationFactory
{
    ...
    private ArchiveTranslationRepository $archiveTranslationRepository;
    ...
    public function __construct(
        ...
        ArchiveTranslationRepository $archiveTranslationRepository,
        ...
        $this->archiveTranslationRepository = $archiveTranslationRepository;
    ...
    private function addGhostLocaleToListElements(array $listeElements, ?string $currentLocale): array {
        ...
        foreach($locales as $locale) {
            $missingLocales = $this->archiveTranslationRepository->findMissingLocaleByIds($ids, $locale, $localesCount);
            foreach($missingLocales as $missingLocale) {
                foreach ($listeElements as $key => $element) {
                    if ($element['id'] === (int)$missingLocale['archive'] && !array_key_exists('ghostLocale', $element)) {
